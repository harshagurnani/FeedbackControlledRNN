...
'''
Analysis of wmp/omp retraining
...
def get_training_results( file='trained_wmp_round2_.npy', folder='wmp/relu_/Model_8_movePC_new__PC8/' , savfolder='wmp/relu_/Model_8_movePC_new__PC8/' , suffix ='', 
                          simple=True, percp=False, hit_thresh=0.1, new_progress=True, use_mod=None, use_fbwt='W_fbk_0' ):
    ''''
    look at wmp/omp retraining results and calculate some statistics. 
    concatenate results across different perturbed decoders ("maps")
    '''
    # Load some stuff   ----------------------------------------------------------------------
    allres = np.load( folder+file, allow_pickle=True ).item()
    train_res = allres['train_res']
    nMaps = len(train_res)          # no. of perturbed mappings
    nReps = len(train_res[0])       # no. of repetitions
    nDim = len(train_res[0][0]['stats']['PC_angle_pre'])    # dim of intrinsic manifold

    # INITIALIZE 
    progress_pre = np.zeros((nMaps,1))              # cursor progress
    progress_post = np.zeros((nMaps,1))
    progress_pre_in = np.zeros((nMaps,1))           # progress due to activity within manifold
    progress_post_in = np.zeros((nMaps,1))
    progress_asymm_pre = np.zeros((nMaps,1))        # progress asymmetry = (max(p)-min(p))/mean(p)
    progress_asymm_post = np.zeros((nMaps,1))
    p_total_target_pre = [np.nan]
    p_total_target_post = [np.nan]
    Xnorm_pre = np.zeros((nMaps,1))                 # activity after perturbation
    Xnorm_post = np.zeros((nMaps,1))                # activity after retraining
    Xnorm_orig = np.zeros((nMaps,1))                # activity with original wout 
    expVar_pre = np.zeros((nMaps,1))                # fractional var within manifold
    expVar_post = np.zeros((nMaps,1))
    expVar_decoder = np.zeros((nMaps,1))            # original variance along decoder
    PC_angle_pre = np.zeros((nMaps,nDim))           
    PC_angle_post = np.zeros((nMaps,nDim))
    hit_rate_pre =  np.zeros((nMaps,1))             # hit rate
    hit_rate_post =  np.zeros((nMaps,1))
    acq_rate_pre =  np.zeros((nMaps,1))             # acquisition time
    acq_rate_post =  np.zeros((nMaps,1))
    im_change = np.zeros((nMaps,1))                 # overlap of variance-per-dim vectors     
    im_angles = np.zeros((nMaps,nDim))              # angles between new activity PCs
    

    # alignment between weights
    ...
    theta_intuit_wperturb = np.zeros((nMaps,1))             # angle between intuitive and perturbed decoder
    theta_base_wperturb = np.zeros((nMaps,1))               # angle between original and perturbed decoder
    theta_im_wperturb = np.zeros((nMaps,1))                 # angle between intrinsic manifold and perturbed decoder
    overlap_base_wperturb = np.zeros( (nMaps,1))            # similarity (overlap)
    overlap_intuit_wperturb = np.zeros((nMaps,1))
    overlap_influence = np.zeros((nMaps,1))                 # overlap of PC influence

    speed = np.zeros((nMaps,1))                             # logistic fit to learning curve - speed
    fitted_k = np.zeros((nMaps,1))                          # logistic rate parameter
    angInp = np.zeros((nMaps,1))
    meanInp = np.zeros((nMaps,1))                           
    meanFbk = np.zeros((nMaps,1))

    alpha = np.zeros((nMaps,1))                             # observability of decoder
    beta_ff = np.zeros((nMaps,1))                           # ff controllability of decoder
    ...
    fracVF_ff = np.zeros((nMaps,1))                         # ff-driven change (wt-driven)
    fracVF_fb = np.zeros((nMaps,1))                         # fb-driven change (wt-driven)
    fracVF_rec = np.zeros((nMaps,1))                        # rec-driven change
    fracOutvel = np.zeros((nMaps,1))                        # change along decoder
    fracSVF_rec = np.zeros((nMaps,1))                       # state-driven change in vf
    ...
    loss_speed = np.zeros((nMaps,1))                        # exponential fitted to loss curve - speed
    loss_fitted_k = np.zeros((nMaps,1))                     # exponential rate parameter

    nTr = train_res[0][0]['posttrain']['lc'].shape[0]       # no. of training trials
    dim_dw_fbk = np.zeros((nMaps,1))                        # dimensionality of change in Wfbk
    corr_dw_fbk = np.zeros((nMaps,1))                       # trial-to-trial correlation in dWfbk
    grad_total = np.zeros((nMaps,nTr))                      # gradient magnitude
    ...
    for map in range(nMaps):
        ...
        # alignment between weights
        wout=wmp_train['posttrain']['params1']['W_out_0']               # perturbed decoder
        wrec_pre=wmp_train['posttrain']['params0']['W_rec_0']           # recurrent wts
        wrec_post=wmp_train['posttrain']['params1']['W_rec_0']
        if simple:
            fb_pre=wmp_train['posttrain']['params0']['W_fbk_0']         # feedback wts
        ...
        # angles with intuitive decoder
        wout_intuit = wmp_train['intuitive']['wout']                # intuitive decoder
        upc = wmp_train['intuitive']['PCRes'].components_.T         # intrinsic manifold
        theta_intuit_wperturb[map] = np.rad2deg(np.min(ll.subspace_angles(wout,wout_intuit)))
        theta_im_wperturb[map] = np.rad2deg(np.min(ll.subspace_angles(wout,upc)))
        ...
        # learning curves
        spd, k, res = return_speed_learning( train_res[map], idx=-1, avg_window=20, use_mod=use_mod, use_fbwt=use_fbwt )    # use all trainng iterations
        speed[map] = spd
        fitted_k[map] = k
        angInp[map] = np.mean( np.abs(res['inAngle']) )
        meanInp[map] = res['inDelta']
        meanFbk[map] = res['fbDelta']
        ...
        expVar_post[map] = mm.get_variance_in_U(X_post, umap)
        expVar_decoder[map] = mm.get_variance_in_U(dic_orig['res']['activity1'], wout )

        # intrinsic manifold similarity
...
def return_eig( params ):
    '''' eigenvalues of closed loop dynamics '''
    Wrec = params['W_rec_0']
    Wout = params['W_out_0']
    Wfb = params['W_fbk_0']
    nNeu = Wrec.shape[0]
...
def return_base_angle( file, wmp_train ):
    ...
    newExp.params['jump_amp']=0.0
    dic = newExp.test_model()           # test model
    wout_base = orig_p['W_out_0']       # original decoder

    wperturb = wmp_train['posttrain']['params1']['W_out_0'] #perturbed decoder
...
def return_speed_learning( wmp_train_map, idx=-1, avg_window=20, use_mod=None, use_fbwt='W_fbk_0' ):
    ''' fit logistic fn to average learning curve '''
    ...
    hitrate = np.mean(allh, axis=1) # average across iterations

    res = mm.fit_logit( uniform_filter1d(hitrate, size=avg_window) )    # fit to smoothed curve
    speed =  res['speed']
    fitted_k = res['fitted_k']
