...
'''
Training function for the 2D reaching task
...
def train(modelt, optimizert, criteriont, tt, targett, stimt, trajt=None, perturb=None, dt=1, 
    ...
    if modelt.W_rec.requires_grad and (not modelt.W_fbk.requires_grad):
    ...
    for epoch in range(tt):
        ...
        #    output,r0 = modelt(dt, stimt[epoch].to(device), targett[epoch].to(device), perturb[epoch].to(device))
        #else:
        #    output,r0 = modelt(dt, stimt[epoch].to(device), targett[epoch].to(device))
        if perturb is not None:
        ...
        #if trajt is None:
        #    loss = criteriont(output[loss_t:loss_maxt], targett[epoch,loss_t:loss_maxt].to(device))        # here target is same as trajectory (e.g. for continuous tracking?) 
        #else:
        #    loss = criteriont(output[loss_t:loss_maxt], trajt[epoch,loss_t:loss_maxt].to(device))          # use trajectory explicity 
        if trajt is None:
        ...
        #dist_from_target = output[hit_period:] -  targett[epoch,hit_period:].to(device)
        ...
        regrec = gam0*modelt.W_rec.norm(2)      # weight L2 regularization for rec
        ...
        if modelt.W_fbk.requires_grad:
            ...
            corr = corr.item()
        elif modelt.W_rec.requires_grad:
            dw_rec = modelt.W_rec.grad.detach()
            corr = 0
            for id in range(modelt.W_rec.shape[0]):
                corr+= torch.dot( dw_rec[id,:],dw_rec_0[id,:])/(torch.norm(dw_rec_0[id,:])*torch.norm(dw_rec[id,:]))
            corr = corr/modelt.W_rec.shape[0]
            dw_rec_0 = dw_rec.detach().clone()
            toprint['corr_grad_rec'] = corr
            corr = corr.item()    
        ...
        if modelt.W_in.requires_grad:
            ...
            corr_in = corr_in/modelt.W_in.shape[0]
...
def model_loss(modelt, criteriont, output, r0, targeto,
                    alp0=1e-3, bet0=1.9*1e-3, gam0=1e-4,                              # regularisation parameters
                    alp1=1e-3, bet1=1.9*1e-3, gam1=1e-4, 
                    clipv=0.2, loss_mask=None, loss_t=20, loss_maxt=800,
                    dist_thresh = 0.15, hit_period=200, print_t=1 ):
    ...
    # calculate loss only between loss_t and loss_maxt - can add mask later 
    loss = criteriont(output[loss_t:loss_maxt], targeto[loss_t:loss_maxt])        # here target is same as trajectory (e.g. for continuous tracking?) 
    
    #dist_from_target = output[hit_period:] -  targett[epoch,hit_period:].to(device)
    dist_from_target = output[hit_period:] -  targeto[hit_period:]
    # make sure not collecting gradients here:
    dist_from_target = dist_from_target.cpu().detach().numpy() # to cpu
    dist_from_target = ll.norm(dist_from_target, axis=2)
    hittarget = np.sum((dist_from_target < dist_thresh ), axis=0) # if dist less at any time
    ...
    #  term 3: submodule parameters?
    if modelt.ff_has_hidden:               #'Perceptron' in modelt.ff_type:
        reg_ff_mod = alp1 * modelt.ff_mod.hidden.weight.norm(2)
        toprint['ff_mod'] = reg_ff_mod
        loss = loss + reg_ff_mod
    
    if modelt.fb_has_hidden:               #'Perceptron' in modelt.fb_type:
        reg_fb_mod = alp1 * modelt.fb_mod.hidden.weight.norm(2)
        toprint['fb_mod'] = reg_fb_mod
        loss = loss + reg_fb_mod
...
####--------
def train_WP(modelt, lossfn, criteriont, tt, targett, stimt, trajt=None, perturb=None, dt=1, eta=0.05,
             train_out=False, train_hid=False, out_sigma=0.05, hid_sigma=0.05, train_rec=False, rec_sigma=0.05,
             train_inp=True, inp_sigma=0.05, train_fbk=True, fbk_sigma=0.05,
          alp0=1e-3, bet0=1.9*1e-3, gam0=1e-4,                              # regularisation parameters
          alp1=1e-3, bet1=1.9*1e-3, gam1=1e-4, 
          clipv=0.2, loss_mask=None, loss_t=20, loss_maxt=800,
          dist_thresh = 0.15, hit_period=200, print_t=1 ):
    ...
    for epoch in range(tt):
        ...
        if trajt is None:
            targeto=targett[epoch]
        else:
            targeto = trajt[epoch]          # use trajectory explicity 
        loss_pre, train_running_loss, toprint = lossfn(modelt, criteriont, output, r0, targeto,
                    alp0=alp0, bet0=bet0, gam0=gam0, alp1=alp1, bet1=bet1, gam1=gam1, clipv=clipv, loss_mask=loss_mask,
                    loss_t=loss_t,  loss_maxt=loss_maxt, dist_thresh=dist_thresh , hit_period=hit_period )        # here target is same as trajectory (e.g. for continuous tracking?) 
        ...
        # perturb weights
        dic = modelt.save_parameters()
        if train_rec:
            Wpert_rec = np.random.randn( dic['W_rec_0'].shape[0], dic['W_rec_0'].shape[1] )*rec_sigma
            dic['W_rec_0'] = dic['W_rec_0'] + Wpert_rec
        if train_inp:
            Wpert_in = np.random.randn( dic['W_in_0'].shape[0], dic['W_in_0'].shape[1] )*inp_sigma
            dic['W_in_0'] = dic['W_in_0'] + Wpert_in
        if train_fbk:
            Wpert_fbk = np.random.randn( dic['W_fbk_0'].shape[0], dic['W_fbk_0'].shape[1] )*fbk_sigma
            dic['W_fbk_0'] = dic['W_fbk_0'] + Wpert_fbk
        if train_out:
            dic['fbk_p']['output.weight'] = dic['fbk_p']['output.weight'].detach().cpu().numpy()
            Wpert_out = np.random.randn( dic['fbk_p']['output.weight'].shape[0], dic['fbk_p']['output.weight'].shape[1] )*out_sigma
            dic['fbk_p']['output.weight'] = dic['fbk_p']['output.weight'] + Wpert_out
        if train_hid:
            dic['fbk_p']['hidden.weight'] = dic['fbk_p']['hidden.weight'].detach().cpu().numpy()
            Wpert_hid = np.random.randn( dic['fbk_p']['hidden.weight'].shape[0], dic['fbk_p']['hidden.weight'].shape[1] )*hid_sigma
            dic['fbk_p']['hidden.weight'] = dic['fbk_p']['hidden.weight'] + Wpert_hid
        ...
        loss_post, _, _ = lossfn(modelt, criteriont, output, r0, targeto,
                    alp0=alp0, bet0=bet0, gam0=gam0, alp1=alp1, bet1=bet1, gam1=gam1, clipv=clipv, loss_mask=loss_mask,
                    loss_t=loss_t,  loss_maxt=loss_maxt, dist_thresh=dist_thresh , hit_period=hit_period )        # here target is same as trajectory (e.g. for continuous tracking?) 

        #  -------> optimizer updates parameters
        # deltaW = -eta/sigma^2 * error * pert
        error = loss_post-loss_pre
        error = error.detach().cpu().numpy()
        if train_rec:
            dic['W_rec_0'] = dic['W_rec_0'] - Wpert_rec - eta/rec_sigma**2 * error * Wpert_rec 
        if train_inp:
            dic['W_in_0'] = dic['W_in_0'] - Wpert_in - eta/inp_sigma**2 * error * Wpert_in 
        if train_fbk:
            dic['W_fbk_0'] = dic['W_fbk_0'] - Wpert_fbk - eta/fbk_sigma**2 * error * Wpert_fbk
        if train_out:
            dic['fbk_p']['output.weight'] =dic['fbk_p']['output.weight'] - Wpert_out - eta/out_sigma**2 * error * Wpert_out
        if train_hid:
