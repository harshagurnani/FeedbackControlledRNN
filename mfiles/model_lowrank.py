...
Recurrent connectivity is lowrank

model parameters:
    - m-vectors and n-vectors to generate recurrent weights
    - input weights
    - feedback weights
    - nonlinearity
    - biases
...
class RNN_LowRank(nn.Module):
    def __init__(self, n_inputs, n_outputs, n_neurons, tau, dtype, rank=1,          # architecture parameters
        ...
        self.rank = rank                    # rank of recurrent weight matrix
    ...
    def init_weights( self, learn_inp=True, learn_rec=True, learn_fbk=True, W_in_0=None, W_rec_0=None, W_fbk_0=None, add_bias_n=False, bias_n=None):
        ...
        self.m_vectors = nn.Parameter( torch.Tensor(self.n_neurons, self.rank).to(dtype=self.dtype, device=self.device))
        self.m_vectors.requires_grad = learn_rec
        self.n_vectors = nn.Parameter( torch.Tensor(self.n_neurons, self.rank).to(dtype=self.dtype, device=self.device))
        self.n_vectors.requires_grad = learn_rec
        self.W_rec = torch.zeros(self.n_neurons, self.n_neurons).to(dtype=self.dtype, device=self.device)#.to(device=self.device) ) # recurrent weights -> trained
        ...
        with torch.no_grad():
            ...
            self.m_vectors.normal_(std=self.init_params['sig_rec_0']/sqrt(self.n_neurons))
            self.n_vectors.normal_(std=self.init_params['sig_rec_0']/sqrt(self.n_neurons))
            self.make_Wrec()
    ...
    def make_Wrec( self ):
        self.W_rec = torch.zeros(self.n_neurons, self.n_neurons).to(dtype=self.dtype, device=self.device)
        for jj in range(self.rank):
            self.W_rec = self.W_rec + torch.outer(self.m_vectors[:,jj], self.n_vectors[:,jj])
    ...
    def forward(self, dt, stim, target, perturb=None):
        ...
        for tm in range( tsteps ):
            ...
            cursor0[tm] = velocity*(dt/1000) + currpos  + perturb[tm]   # in cm, (dt is in ms)
            currpos = cursor0[tm]
    ...
    def load_parameters(self, params):
        ...
        state_dict['m_vectors'] = torch.Tensor( params['m_vectors']).to(dtype=self.dtype, device=self.device)  
        state_dict['n_vectors'] = torch.Tensor( params['n_vectors']).to(dtype=self.dtype, device=self.device)  
    ...
    def save_parameters(self):
        ...
        mvec = self.m_vectors.cpu().detach().numpy().copy()
        nvec = self.n_vectors.cpu().detach().numpy().copy()
        ...
        dic = {'W_in_0':win,'W_rec_0':wrec, 'W_out_0':wout, 'W_fbk_0':wfbk, 
               ...
               'W_in_mask':win_mask, 'W_fbk_mask':wfb_mask, 'm_vectors':mvec, 'n_vectors':nvec }
