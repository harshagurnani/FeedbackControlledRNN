...
for nz in allz:
       idx = (currentz==nz)
       cev_summary = np.concatenate((cev_summary, all_cev[idx]), axis=0)
       nz_summary = np.concatenate((nz_summary, currentz[idx]), axis=0)
...
idx=np.intersect1d( np.where(all_cev>min_cev), np.where(currentz==nz) )
eigs = all_ev[idx]

rot_ev_all = np.zeros(0,)
for ee in eigs:
       ...
       if len(nonzero_imaginary_indices) > 0:
              slowest_eigenvalue_index = np.argmax(real_part[nonzero_imaginary_indices])
              slowest_eigenvalue = ee[nonzero_imaginary_indices[slowest_eigenvalue_index]]
              rot_ev_all = np.concatenate((rot_ev_all, slowest_eigenvalue), axis=0)

pp.figure()
pp.hist(np.real(rot_ev_all), bins=np.linspace(-1, 1, 20), color='r', alpha=0.3, label='Real')
pp.hist(np.imag(rot_ev_all), bins=np.linspace(-1, 1, 20), color='r', alpha=0.3, label='Imaginary')
...
rotational_frequency = np.imag(rot_ev_all) / (2 * np.pi * sampling_time)
