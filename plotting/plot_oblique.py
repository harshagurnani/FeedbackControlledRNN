...
topfolder = 'percp2_expansion_/'#'relu_noFbkNeu_/'#'relu_amp_/' #'relu_sparse_neu_/'#'relu_layer1/'#'linear_gpu/' #'simple_gpu/'
savfolder = 'oblique_dynamics/'+topfolder
file  = savfolder+'wout_angles.npy'
allAng = np.load( file, allow_pickle=True )

nfiles = len( allAng )
train_keys = ['Out_Fbk', 'Out_PCs', 'Out_RightSV', 'ID_PCs']
intuit_keys = ['Out_Intuitive', 'Out_Intrinsic' ]
bkeys = ['sig_out_0', 'sig_rec_0']
...
if np.sum(SparseNeu)>0:
    Wfbk = np.multiply(Wfbk, SparseNeu)
elif np.sum(SparseWt)>0:
    Wfbk = np.multiply(Wfbk, SparseWt)
...
goodtrain = (HitRate_T_I[usef,0]>0.3)            # plot all or only successfully trained models???
