...
'''
Batch Script - generate a bunch of RMPs and save their properties, including for filtering
Usage:
    python batch_gen_rmp.py [-h] [-F FOLDER] [-sf SUFFIX] [-nf NFILES] [-idx INDEX] [-sign SIGMA_N] [-pc NPC_FIT] [-tF FIT_MAXT] [-tM TEST_MAXT] [-nlist NLIST_SUB] [-U0 U_ANG0] [-U1 U_ANG1] [-ratio0 RATIO_LIM0] [-ratio1 RATIO_LIM1] [-vel0 VELRANGE0] [-vel1 VELRANGE1] [-tm0 USETM0] [-tm1 USETM1]
...
# parameters:
def sweep_rmp(loadname='use_models/relu_', savfolder='rmp/relu_', n1files=None, maxfiles=10, suffix = '_PC8', save_pn = True, index=1, 
              fit_maxT = 1000,  test_maxT=1500, noiseX=0.1, nTest=200, nPC_fit=8,  sigma_n=0.0, 
              nList=1e5, nList_sub=5000, ratio_lim=[0.5,2], velAngle=[30,75], tm_range=[200,500] , device=None ):
    
    print('...Starting RMP Sweep for models in '+loadname+' ...')
    # all model files
    if n1files is None:
    ...
    wmp_p = {'nList_p':nList, 'nList_sub':nList_sub, 'ratio_lim':ratio_lim, 'velAngle':velAngle}         # how many maps to sample? criteria for wmp
    ...
    for ff in range(nf):
        ...
        # collect rmps:
        ca, speed, angle, medangle, pperm, mapids, ratio = newExp.generate_random_decoders( ch_params=wmp_p )
        z = (medangle * angle ) * speed # all criteria pass or not
        usemap= [mapids[xx] for xx in range(len(z)) if z[xx]]
        rdic.update({'wout_angle':ca, 'mean_velangle':angle, 'median_velangle':medangle, 'speed':speed, 
                    'perm':pperm, 'rank_id_0':mapids, 'sratio':ratio, 'npass':sum(z), 'use_rmp_id':usemap, 
                    'ang_int_wout':ang_int_wout })


        good_maps, allspeed, perfrate = newExp.post_filter_wmp( use_maps=usemap, list_perm=pperm, plot_num=10, savpath=subf, get_perf=True, maptype='rmp', use_vratio=False )
        rdic.update({'good_maps':good_maps, 'closedv':allspeed, 'perfrate':perfrate})
        newExp.rmp_maps = {'good_maps':good_maps, 'closedv':allspeed, 'perfrate':perfrate}
        print(len(good_maps))
        if save_pn:
            np.save(subf+'RMP_Maps', {'newExp':newExp, 'intuitive':rdic['intuitive']})

        results.append( rdic )

    np.save( savfolder+'rmp_tested_movepc'+suffix, results)        # movepc for fit
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Sweep across Random Decoder Perturbations (RMPs) and determine their properties and goodness')
    ...
    parser.add_argument('-nlist', '--nList_sub', type=int, default=1000, help='Max number of random RMPs to check') 
    parser.add_argument('-nlist_p', '--nList_search', type=int, default=100000, help='Max number of random RMPs to search originally') 
    ...
    sweep_rmp(loadname=loadname, savfolder=savfolder, maxfiles=args.nfiles, fit_maxT=args.fit_maxT, test_maxT=args.test_maxT , index = args.index,
               nList=args.nList_search, nList_sub=args.nList_sub,ratio_lim=ratio_lim, velAngle=velAngle, tm_range=tm_range, suffix=suffix, nPC_fit=args.nPC_fit, sigma_n=args.sigma_n )